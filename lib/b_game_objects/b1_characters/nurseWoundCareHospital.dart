...
class NurseWoundCareHospital extends Perso
    with HasGameRef<DiabeteGameSceneNurseOffice> {
  NurseWoundCareHospital(String user)
      : super(fileNamePngCharacter: GameImageAssets.nurseWoundCareHospital);
      ...
      if (!_hasCollided) {
        if (gameRef.step1) {
          ...
          gameRef.gameScenesController.gameDialogController.onDialog(
            DialogModel(
                isShowDialog: true,
                dialogMessage: 'You should talk to the diabetes nurse first'),
          );
