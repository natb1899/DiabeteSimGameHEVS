...
void main() {
  ...
  DiabeteGameSceneFraiseuxChamber diabeteGame = DiabeteGameSceneFraiseuxChamber(
  ...
  group('Scene 5 tests', () {
    testWithGame<DiabeteGameSceneFraiseuxChamber>(
        'Load correct tile', () => diabeteGame,
        (DiabeteGameSceneFraiseuxChamber game) async {
      await game.ready();
      expect(game.sceneTmx, GameTilesAssets.fraiseuxChamber);
      expect(game.sceneName, GameScenes.atFraiseuxChamber);
    });
    testWithFlameGame('Load player in scene 5', (game) async {
      var player = PlayerComponent('player48x48.png')..addToParent(game);
      await game.ready();
      expect(player.isMounted, true);
    });
    group("load npc", () {
      testWithFlameGame('Mr. Fraiseux', (game) async {
        final component = Fraiseux("")..addToParent(game);
        await game.ready();
