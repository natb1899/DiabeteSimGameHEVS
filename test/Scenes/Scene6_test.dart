...
import 'package:seriousgame/b_game_objects/b1_characters/choux.dart';
...
void main() {
  ...
  DiabeteGameSceneChouxChamber diabeteGame = DiabeteGameSceneChouxChamber(
  ...
  group("Scene 6 test", () {
    testWithGame<DiabeteGameSceneChouxChamber>(
        'Load correct tile', () => diabeteGame,
        (DiabeteGameSceneChouxChamber game) async {
      await game.ready();
      expect(game.sceneTmx, GameTilesAssets.chouxChamber);
      expect(game.sceneName, GameScenes.atChouxChamber);
    });
    testWithFlameGame('Load player in scene 6', (game) async {
    ...
    group("load npc", () {
      testWithFlameGame('Mme. Choux', (game) async {
        final component = Choux()..addToParent(game);
        await game.ready();
